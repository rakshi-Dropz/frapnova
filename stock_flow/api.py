...
def create_stock_order(item_code, quantity=1):
    ...
    # 1. Validation: Prevent placing order if requested qty exceeds available stock
...
def reject_stock_order(order_id):
    """Marks a draft order as cancelled/rejected in the database without altering stock."""
    if frappe.session.user == "Guest":
    ...
    # 2. Check if the DocType has a 'status' column, and update status + docstatus cleanly
